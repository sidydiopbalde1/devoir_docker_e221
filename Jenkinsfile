...
pipeline {
  ...
  environment {
    // Définir les identifiants Docker Hub et l'URL du webhook Render
    // Ces credentials doivent être configurés dans Jenkins (Type: Username with password pour DOCKERHUB, Secret text pour RENDER_DEPLOY_HOOK)
    DOCKERHUB_CREDS = credentials("dockerhub-creds") // ID du credential Docker Hub (Username + Password)
    RENDER_DEPLOY_HOOK = credentials("render-webhook") // ID du credential Secret Text pour le webhook Render
    IMAGE_NAME = "${DOCKERHUB_CREDS_USR}/mon_application_ci_cd" // Nom de l'image Docker, ajustez si nécessaire
  }
  ...
  post {
    ...
    success {
      ...
      // mail to: EMAIL_RECIPIENTS, subject: "Jenkins Build Succeeded: ${env.JOB_NAME} #${env.BUILD_NUMBER}", body: "Build ${env.BUILD_NUMBER} of ${env.JOB_NAME} succeeded. Check ${env.BUILD_URL}"
    }
    failure {
      echo "❌ Échec: Le pipeline a échoué."
      // Envoyer une notification par email en cas d'échec
      // mail to: EMAIL_RECIPIENTS, subject: "Jenkins Build Failed: ${env.JOB_NAME} #${env.BUILD_NUMBER}", body: "Build ${env.BUILD_NUMBER} of ${env.JOB_NAME} failed. Check ${env.BUILD_URL}"
    }
  }
}
